...
**为了确保系统正常运行，购买后请按以下步骤提供服务器硬件信息：**

#### 1. 下载硬件信息采集脚本

```bash
# 下载脚本（供应商会提供）
wget https://your-domain.com/get_hardware_info.sh
...
主板UUID: 12345678-1234-1234-1234-123456789012
...
1. **主板UUID**: `12345678-1234-1234-1234-123456789012`
2. **网卡名称**: `eth0` （选择一个主网卡，通常是 eth0、ens33、ens160 等）
3. **网卡MAC地址**: `00:11:22:33:44:55`

**注意事项**：
- 必须在实际部署的服务器上运行脚本
- 如果有多个网卡，请选择主网卡（通常是连接外网的网卡）
- 请准确记录网卡名称，部署时需要使用
- 更换服务器需要重新提供硬件信息并获取新的授权版本
